...
# output_dir='/private/groups/russelllab/jodie/wolbachia_induced_DE/wolbachia_induced_differentiation/scripts/celltype_clustering/claude/MNN_kNN_tree/embryo_atlas_v6'
...
def preprocess_data(bulk_adata, ref_adata, annotation_key):
    ...
    bulk_adata.var_names_make_unique()
    bulk_adata.obs_names_make_unique()
...
def integrate_datasets(bulk_adata, ref_adata):
    ...
    # combined = bulk_adata.concatenate(ref_adata, batch_key="dataset", batch_categories=["Bulk", "Single-cell"])

    logger.info(f"Combining datasets")

    combined = ad.concat([bulk_adata, ref_adata], 
                     label="dataset", 
...
ref_processed=subsample_celltypes(ref_processed, annotation_key) #use tissue as the annotation key for FCA 
...
# plt.tight_layout(rect=[0, 0, 1, 1])  # Leaves space for legend on the right
